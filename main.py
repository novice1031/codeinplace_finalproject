...
def create_index(filenames, index, file_titles):
    ...
    for filename in filenames:
        with open(filename, 'r') as f:
            ...
            for term in terms:
                normalized_term = term.lower().strip(string.punctuation)

                if normalized_term != '' and set(normalized_term) != set(string.punctuation):
                    if normalized_term in index:
                        index[normalized_term].append(filename)
                    else:
...
def search(index, query):
    ...
    # Find the first term in the index
    for term in terms:
        if term in index:
            result = list(set(index[term]))
            break

    # Iterate over the remaining terms and update the result list accordingly
    for term in terms:
        if term in index:
            result = list(set(result) & set(index[term]))
        else:
            # If term is not in index, the intersection will be empty, so we can break early
            result = []
            break
